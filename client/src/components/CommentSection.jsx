...
export default function CommentSection({ postId }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (comment.length > 200) {
      setCommentError("The length of the comment cannot be more than 200");
      return;
    }
    try {
    ...
    <div className="max-w-2xl mx-auto w-full p-3">
      {currentUser ? (
        ...
        <div className="text-sm text-teal-500 my-5">
          You must be signer in to comment.
          <Link className="text-blue-500 hover:underline ml-1" to={"/sign-in"}>
            Sign in
          </Link>
        </div>
      )}
